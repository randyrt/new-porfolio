      ...
      <Teleport to="body">
        <Transition name="modal">
          <div v-if="showTechDetails"
            ...
            <div
              class="relative border border-white/10 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[85vh] overflow-y-auto custom-scrollbar z-49"
              ...
              <div
                ...
                :style="{ backgroundColor: 'color-mix(in srgb, #0a0f1a, #1e3a5f 85%)' }">
              ...
              <div class="p-8 space-y-8">
                <!-- Vue.js 3 -->
                <div
                  class="bg-white/5 border border-white/10 p-6 rounded-2xl border-l-4 border-l-sky-500 transition-all hover:bg-white/[0.07]">
                  <h4 class="font-black text-xl text-white flex items-center gap-3 mb-4">
                    <font-awesome-icon :icon="['fab', 'vuejs']" class="text-sky-500" />
                  ...
                  <p class="text-sm !text-emerald-100 mt-4 leading-relaxed">{{ $t('about.vue_desc') }}</p>
                </div>

                <!-- TypeScript -->
                <div
                  class="bg-white/5 border border-white/10 p-6 rounded-2xl border-l-4 border-l-blue-600 transition-all hover:bg-white/[0.07]">
                  <h4 class="font-black text-xl text-white flex items-center gap-3 mb-4">
                  ...
                  <div
                    class="bg-black/50 border border-white/10 text-emerald-400 p-4 rounded-xl text-xs font-mono mt-4 overflow-x-auto shadow-inner">
                    <pre>type Project = {
  id: string
  title: string
...
const defeatBoss = (id: string): void => { ... }</pre>
                  ...
                  <p class="text-sm !text-emerald-100 mt-4 leading-relaxed">{{ $t('about.ts_desc') }}</p>
                </div>

                <!-- Tests & Qualité -->
                <div
                  class="bg-white/5 border border-white/10 p-6 rounded-2xl border-l-4 border-l-purple-500 transition-all hover:bg-white/[0.07]">
                  <h4 class="font-black text-xl text-white flex items-center gap-3 mb-4">
                ...
                <!-- CI/CD Pipeline -->
                <div
                  class="bg-white/5 border border-white/10 p-6 rounded-2xl border-l-4 border-l-slate-400 transition-all hover:bg-white/[0.07]">
                  <h4 class="font-black text-xl text-white flex items-center gap-3 mb-4">
                    <font-awesome-icon :icon="['fab', 'github']" class="text-white" />
                    <span>{{ $t('about.ci_cd') }}</span>
                  </h4>
                  <div
                    class="bg-black/50 border border-white/10 !text-emerald-100 p-4 rounded-xl text-xs font-mono mt-4 shadow-inner">
  ...
  <span class="text-blue-400">deploy:</span>
    <span class="text-slate-500">steps:</span>
      ...
      - netlify-push <span class="text-emerald-500">✓</span></pre>
                  ...
                  <p class="text-sm !text-emerald-100 mt-4 leading-relaxed">{{ $t('about.ci_desc') }}</p>
                </div>

                <!-- Summary Stats -->
                <div
                  class="relative overflow-hidden p-6 rounded-2xl bg-gradient-to-br from-sky-600/15 via-slate-800 to-slate-900 border border-white/10">
                ...
                <div class="text-center text-xs text-slate-500 border-t border-white/10 pt-8 space-y-2">
