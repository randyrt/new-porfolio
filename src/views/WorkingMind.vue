<template>
    ...
    <div v-else class="p-4 w-full flex flex-col items-center justify-center max-w-6xl mx-auto pb-12">
        ...
        <section class="w-full mb-10 rounded-3xl border card p-6 shadow-sm" data-aos="fade-up">
            <div class="flex items-center gap-3 mb-8">
                <span
                    class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-violet-100 text-violet-700 font-bold">↗</span>
                <div>
                    <p class="text-[10px] uppercase tracking-[0.25em] text-violet-500 font-bold">{{
                        $t('workingmind.timeline_label') }}</p>
                ...
                <div v-for="item in timeline" :key="item.period" class="relative pl-12">
                    ...
                    <div class="rounded-2xl border border-violet-50 bg-violet-50/60 p-4">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-2">
                            <span class="text-xs font-bold uppercase tracking-[0.2em] text-violet-600">{{ item.period
                                }}</span>
        ...
        <section class="w-full mb-10 rounded-3xl border card p-6 shadow-sm">
            <div class="mb-6">
                <p class="text-[10px] uppercase tracking-[0.25em] text-violet-500 font-bold">{{
                    $t('workingmind.featured_case_study') }}</p>
            ...
            <div class="grid lg:grid-cols-[1.4fr_0.8fr] gap-6 items-stretch">
                <div class="rounded-2xl bg-white p-5 shadow-sm border border-violet-100">
                    ...
                    <div class="grid sm:grid-cols-3 gap-3 text-center mb-5">
                        <div v-for="metric in featuredCaseStudy.metrics" :key="metric.label"
                    ...
                    <div class="space-y-4 text-sm">
                        <div>
                            <p class="font-bold text-slate-800 uppercase tracking-[0.15em] text-[10px]">{{
                                $t('workingmind.featured_challenge') }}</p>
                            <p class="mt-1 text-slate-600">{{ featuredCaseStudy.challenge }}</p>
                        </div>
                        <div>
                            <p class="font-bold text-slate-800 uppercase tracking-[0.15em] text-[10px]">{{
                                $t('workingmind.featured_solution') }}</p>
                ...
                <div class="rounded-2xl bg-slate-900 p-5 text-white shadow-lg">
                    <p class="text-[10px] uppercase tracking-[0.2em] text-violet-300 font-bold">{{
                        $t('workingmind.featured_stack') }}</p>
                    <div class="mt-4 flex flex-wrap gap-2">
                        <span v-for="tech in featuredCaseStudy.stack" :key="tech"
                    ...
                    <div class="mt-6 rounded-xl border border-white/10 bg-white/5 p-4">
                        <p class="text-[10px] uppercase tracking-[0.2em] !text-cyan-300 font-bold">{{
                            $t('workingmind.featured_result') }}</p>
        ...
        <section class="w-full rounded-3xl border card p-6 shadow-sm">
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                    <p class="text-[10px] uppercase tracking-[0.25em] text-violet-500 font-bold">{{
                        $t('workingmind.personality_label') }}</p>
            ...
            <div class="mt-6 grid md:grid-cols-3 gap-4">
                <div v-for="trait in personality" :key="trait.title"
