<template>
  <div class="construction-3d h-64 bg-slate-950/80 rounded-xl border border-white/5 relative overflow-hidden group">
    ...
    <!-- Controls Overlay -->
    <div class="absolute bottom-4 left-4 right-4 flex items-center gap-4 bg-black/40 backdrop-blur-md p-3 rounded-lg border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity">
      <button 
        @click="togglePlay" 
      ...
      <div class="flex-1 relative h-1.5 bg-white/10 rounded-full">
        ...
        />
        <div class="h-full bg-indigo-400 rounded-full transition-all" :style="{ width: `${(playbackIndex / (commits.length - 1)) * 100}%` }"></div>
      </div>
    ...
    <Transition name="fade">
      <div v-if="currentCommit" class="absolute top-4 right-4 text-right">
        <p class="text-[10px] text-white/40 uppercase tracking-widest mb-1">{{ $t('git_time_machine.current_mood') }}</p>
        <div class="flex items-center gap-2">
            <span class="text-xl">{{ getEmotionEmoji(currentCommit.emotion) }}</span>
            <span class="text-xs font-bold" :class="getEmotionTextColor(currentCommit.emotion)">{{ currentCommit.emotion.toUpperCase() }}</span>
        </div>
      </div>
    </Transition>
  </div>
